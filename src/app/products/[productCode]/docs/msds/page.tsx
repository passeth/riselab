...
import { ArrowLeft, Loader2, AlertCircle, AlertTriangle, ExternalLink, FileText, Globe } from "lucide-react";
...
export default function MsdsPage() {
  ...
  const [ingredients, setIngredients] = useState<IngredientWithMsds[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [langFilter, setLangFilter] = useState<"all" | "kr" | "en">("all");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 3. labdoc_ingredients에서 MSDS URL 조회
      const { data: ingredientsData, error: ingErr } = await supabase
        .from("labdoc_ingredients")
        .select("ingredient_code, ingredient_name, msds_kr_urls, msds_en_urls")
      ...
      // 매핑
      type IngredientRow = { ingredient_code: string; ingredient_name: string; msds_kr_urls: string[] | null; msds_en_urls: string[] | null };
  ...
  const filteredIngredients = ingredients.filter((ing) => {
    const hasKr = ing.msdsKrUrls.length > 0;
    const hasEn = ing.msdsEnUrls.length > 0;
    if (langFilter === "kr") return hasKr;
    if (langFilter === "en") return hasEn;
    return hasKr || hasEn; // all - MSDS가 있는 것만
  });

  const ingredientsWithMsds = ingredients.filter((ing) => ing.msdsKrUrls.length > 0 || ing.msdsEnUrls.length > 0);
  const ingredientsWithoutMsds = ingredients.filter((ing) => ing.msdsKrUrls.length === 0 && ing.msdsEnUrls.length === 0);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-800">MSDS (물질안전보건자료)</h2>
          <p className="text-sm text-slate-500 mt-0.5">
            {product.korean_name || product.english_name}
            <span className="ml-2 text-slate-400">
              ({ingredientsWithMsds.length}/{ingredients.length} 원료에 MSDS 존재)
            </span>
          </p>
        </div>
        <Link
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-slate-700">원료별 MSDS 문서</h3>
            <span className="text-xs text-slate-400">{filteredIngredients.length}건</span>
          </div>
          {/* 언어 필터 */}
        ...
        {ingredients.length === 0 ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-slate-400 text-sm">
              {langFilter === "all" 
                ? "등록된 MSDS 문서가 없습니다" 
                : `${langFilter === "kr" ? "국문" : "영문"} MSDS 문서가 없습니다`}
            </p>
          </div>
        ) : (
              ...
              <div key={ing.ingredientCode} className="px-4 py-3 hover:bg-amber-50/30">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-wrap gap-2 justify-end">
                    {/* 국문 MSDS */}
                    {(langFilter === "all" || langFilter === "kr") && ing.msdsKrUrls.map((url, idx) => (
                      <a
                        key={`kr-${idx}`}
                        href={url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-600 text-xs rounded hover:bg-blue-100 transition-colors"
                      >
                        <FileText size={12} />
                        국문 {ing.msdsKrUrls.length > 1 ? `#${idx + 1}` : ""}
                        <ExternalLink size={10} />
                      </a>
                    ))}
                    {/* 영문 MSDS */}
                    {(langFilter === "all" || langFilter === "en") && ing.msdsEnUrls.map((url, idx) => (
                      <a
                        key={`en-${idx}`}
                        href={url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 px-2 py-1 bg-purple-50 text-purple-600 text-xs rounded hover:bg-purple-100 transition-colors"
                      >
                        <Globe size={12} />
                        영문 {ing.msdsEnUrls.length > 1 ? `#${idx + 1}` : ""}
                        <ExternalLink size={10} />
                      </a>
                    ))}
                  </div>
      ...
      {/* MSDS 없는 원료 (접힌 섹션) */}
      {ingredientsWithoutMsds.length > 0 && (
        <details className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
          <summary className="px-4 py-3 border-b border-slate-100 flex items-center gap-2 cursor-pointer hover:bg-slate-50">
            ...
            <h3 className="text-sm font-medium text-slate-500">MSDS 미등록 원료</h3>
            <span className="ml-auto text-xs text-slate-400">{ingredientsWithoutMsds.length}건</span>
          </summary>
          <div className="divide-y divide-slate-50">
            {ingredientsWithoutMsds.map((ing) => (
