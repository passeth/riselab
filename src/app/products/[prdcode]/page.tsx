...
import { ArrowLeft, Loader2, AlertCircle, Edit2, FileSpreadsheet, List, FlaskConical, Beaker } from "lucide-react";
import type { LabProduct, IngredientComponent } from "@/types/database";
...
interface ProductInfo extends LabProduct {
...
export default function ProductDetailPage() {
  const { prdcode } = useParams<{ prdcode: string }>();
  const decodedPrdcode = decodeURIComponent(prdcode);
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      // 1. lab_products에서 제품 정보 조회
      const { data: productData, error: productErr } = await supabase
        .from("lab_products")
        .select("*")
        .eq("prdcode", decodedPrdcode)
        .single();
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-slate-800 tracking-tight">{product.product_name}</h1>
        <div className="flex items-center gap-4 mt-2 flex-wrap">
          <p className="text-sm text-slate-400">
            품목코드: <span className="font-mono text-slate-600">{decodedPrdcode}</span>
          </p>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 mb-5">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
          ...
          <div>
            <span className="text-slate-400 text-xs uppercase">BOM버전</span>
            <p className="text-slate-600">{product.bom_version || "—"}</p>
          </div>
          <div>
